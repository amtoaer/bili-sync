...
bili-sync 是一款专为 NAS 用户编写的视频同步工具，由 Rust & Tokio 驱动，当前支持哔哩哔哩订阅下载，并提供 YouTube 订阅与手动提交下载能力。
...
- [x] 支持在 Web UI 中手动提交 B 站视频链接 / BV 号并立即触发下载
- [x] 支持 YouTube Cookie 配置、频道 / 播放列表订阅，以及单视频链接手动提交下载
- [x] 支持限制任务的并行度和接口请求频率
- [x] 支持单个文件的分块并行下载
...
- 手动下载：可在 Web UI 中直接粘贴 B 站视频链接、BV 号或 YouTube 单视频链接，提交后后台立即开始处理
- YouTube 支持：可导入 Cookie 后订阅频道、播放列表，也可单独手动提交链接进行下载
